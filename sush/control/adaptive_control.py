    ...
    def create_from_dict(config: dict[str, Any]) -> ConditionEvaluator:
        ...
        if condition_type == "threat_level":
            return ThreatLevelCondition(
                operator=config["operator"], threshold=ThreatLevel[config["threshold"]]
            )
            ...
            return PerformanceCondition(
                metric=config["metric"], operator=config["operator"], threshold=config["threshold"]
            )
        elif condition_type == "stability":
            return StabilityCondition(operator=config["operator"], duration=config["duration"])
...
class AdaptiveControlLoop:
            ...
            if self.censorship_detector:
                security_metrics = {
                    ...
                    "detection_confidence": getattr(
                        self.censorship_detector, "last_confidence", 0.0
                    ),
                }
    ...
    def _calculate_baseline(self):
        ...
        self.logger.info(
            f"Baseline metrics calculated: latency={avg_latency:.3f}s, "
    ...
    async def _analyze_threats(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.current_metrics and self.baseline_metrics:
                latency_ratio = self.current_metrics.overall_latency / max(
                    self.baseline_metrics.overall_latency, 0.001
                )
                throughput_ratio = self.current_metrics.throughput / max(
                ...
                if (
                    ...
                    or self.current_metrics.success_rate < 0.8
                ):
            ...
            if len(self.performance_history) > 1:
                ...
                if stable_since:
                    ...
                    threat_analysis["stability_duration"] = (
                        time.time() - self.performance_history[0].timestamp
                    )
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "threat_level": (
                self.censorship_detector.current_threat_level.name
                if self.censorship_detector
                else "UNKNOWN"
            ),
            ...
            "uptime": time.time()
            - (self.performance_history[0].timestamp if self.performance_history else time.time()),
            "components_online": {
