...
class ThreatMonitor:
    ...
    async def _probe_target(self, target: ProbeTarget) -> ProbeResponse:
        ...
        try:
            ...
            return ProbeResponse(
                target=target, result=result, timestamp=start_time, latency=latency
            )
    ...
    async def _http_request_probe(self, target: ProbeTarget) -> ProbeResult:
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(target.host, target.port or 80), timeout=target.timeout
            )
            ...
            response = await asyncio.wait_for(reader.read(1024), timeout=target.timeout)
        ...
        try:
            ...
            reader, writer = await asyncio.wait_for(
                asyncio.open_connection(
                    target.host, target.port or 443, ssl=context, server_hostname=target.host
                ),
                ...
                for domain in self.dns_poison_domains:
                    ...
                    for dns_server in self.dns_servers:
                        try:
                            ...
                            self.logger.debug(
                                f"DNS query failed for {domain} via {dns_server}: {e}"
                            )
    ...
    async def _analyze_blocking_patterns(self):
        ...
        for target, results in target_results.items():
            ...
            if failure_rate > 0.8 and len(results) >= 5:
                self.logger.warning(
                    f"Systematic blocking detected for {target} (failure rate: {failure_rate:.2f})"
                )
    ...
    async def _detect_middleboxes(self):
        ...
        recent_tcp_probes = [
            p
            for p in list(self.probe_history)[-20:]
        ...
        fast_failures = [
            p
            for p in recent_tcp_probes
            if (
                p.result == ProbeResult.CONNECTION_REFUSED and p.latency and p.latency < 0.01
            )  # Very fast response
        ]
    ...
    def get_probe_statistics(self) -> Dict[str, any]:
        ...
        successful_probes = [
            p for p in recent_probes if p.result == ProbeResult.SUCCESS and p.latency
        ]
        avg_latency = (
            sum(p.latency for p in successful_probes) / len(successful_probes)
