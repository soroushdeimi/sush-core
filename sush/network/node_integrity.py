...
class SimplifiedNodeIntegrity:
    ...
    async def register_node(self, node_id: str, public_key: str, endorser: Optional[str] = None) -> bool:
        try:
            ...
            node_record = NodeRecord(node_id=node_id, public_key=public_key, first_seen=time.time(), last_seen=time.time(), reputation_score=initial_reputation, status=NodeStatus.UNKNOWN, endorsements=set(), complaints=set(), uptime_reports=[], bandwidth_reports=[])
    ...
    async def submit_performance_report(self, target_node: str, uptime_percentage: float, avg_latency: float, bandwidth_mbps: float, successful_relays: int, failed_relays: int) -> bool:
        try:
            if target_node not in self.node_registry:
                self.logger.warning(f"Cannot report on unknown node {target_node}")
                return False
            report_data = {'reporter': self.node_id, 'target': target_node, 'timestamp': time.time(), 'uptime': uptime_percentage, 'latency': avg_latency, 'bandwidth': bandwidth_mbps, 'successful_relays': successful_relays, 'failed_relays': failed_relays}
            signature = self._sign_data(json.dumps(report_data, sort_keys=True))
            report = PerformanceReport(reporter_id=self.node_id, target_node=target_node, timestamp=time.time(), uptime_percentage=uptime_percentage, avg_latency=avg_latency, bandwidth_mbps=bandwidth_mbps, successful_relays=successful_relays, failed_relays=failed_relays, signature=signature)
    ...
    async def endorse_node(self, target_node: str) -> bool:
        try:
            ...
            node_record.reputation_score = min(self.max_reputation, node_record.reputation_score + boost)
    ...
    async def report_complaint(self, target_node: str, reason: str) -> bool:
        try:
            ...
            penalty = 0.05
            node_record.reputation_score = max(self.min_reputation, node_record.reputation_score - penalty)
            await self._update_node_status(target_node)
    ...
    def get_trusted_nodes(self) -> List[str]:
        return [node_id for node_id, record in self.node_registry.items() if (record.reputation_score >= self.trust_threshold and record.status not in [NodeStatus.BLOCKED, NodeStatus.SUSPICIOUS])]
    ...
    async def cleanup_old_reports(self, max_age_days: int = 30):
        ...
        self.performance_reports = [report for report in self.performance_reports if report.timestamp > cutoff_time]
    ...
    async def _update_node_reputation(self, node_id: str, report: PerformanceReport):
        ...
        weight = 0.1
        node_record.reputation_score = (node_record.reputation_score * (1 - weight) + performance_score * weight)
    ...
    def get_network_statistics(self) -> Dict[str, Any]:
        ...
        for status in NodeStatus:
            status_counts[status.name] = sum(1 for record in self.node_registry.values() if record.status == status)
        return {'total_nodes': len(self.node_registry), 'avg_reputation': sum(reputations) / len(reputations), 'status_distribution': status_counts, 'trusted_nodes': len(self.get_trusted_nodes()), 'total_reports': len(self.performance_reports)}