...
class QuantumObfuscator:
    ...
    async def initialize_session(
        ...
        network_condition: NetworkCondition = NetworkCondition.MEDIUM_BANDWIDTH,
    ) -> ObfuscationContext:
        """
    ...
    def _create_encrypted_packet(self, ciphertext: bytes, iv: bytes, tag: bytes) -> bytes:
        ...
        packet = iv_size.to_bytes(1, "big") + tag_size.to_bytes(1, "big") + iv + tag + ciphertext
    ...
    async def adapt_to_conditions(
        ...
        if performance_feedback is not None:
            self.cipher_suite.update_state_machine(
                ...
                self.cipher_suite.current_profile.name
                if self.cipher_suite.current_profile
                else "unknown",
        ...
        self.logger.info(
            f"Adapted session {session_id} to threat={threat_level.name}, "
