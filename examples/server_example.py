...
async def run_basic_server():
    ...
    try:
        ...
        while server.is_running:
            ...
            print(
                f"Status: {status['active_connections']} connections, "
    ...
    try:
        ...
        while server.is_running:
            ...
            if status["active_connections"] > 0:
                print(
                    f"Active: {status['active_connections']} connections, "
...
async def run_directory_server():
    ...
    try:
        ...
        while server.is_running:
            ...
            print(
                f"Directory status: {integrity_status['total_nodes']} nodes, "
...
async def run_high_performance_server():
    ...
    try:
        ...
        print(
            f"Optimized for: {config.max_clients} clients, {config.bandwidth_limit / 1_000_000:.0f} Mbps"
        )
        ...
        while server.is_running:
            ...
            if last_stats:
                ...
                print(
                    f"Performance: {mbps:.1f} Mbps, {conn_rate:.1f} conn/s, "
...
async def run_monitoring_server():
    ...
    config = ServerConfig(
        listen_address="0.0.0.0", listen_ports=[8443], is_mirror_node=True, log_level="DEBUG"
    )
    ...
    try:
        ...
        while server.is_running:
            ...
            print(
                f"Connections: {status['active_connections']} active, "
                f"{status['relay_connections']} relays"
            )
...
async def main():
    ...
    parser.add_argument("--address", default="0.0.0.0", help="Listen address (default: 0.0.0.0)")
    parser.add_argument(
        "--ports", nargs="+", type=int, default=[8443], help="Listen ports (default: 8443)"
    )
