...
async def simple_connection_example():
    ...
    config = ClientConfig(log_level="INFO", obfuscation_level=0.8, enable_steganography=True)
...
async def multi_connection_example():
    ...
    config = ClientConfig(max_concurrent_connections=5, obfuscation_level=0.6)

    async with SushClient(config) as client:
        connections = []

        # Create multiple connections
        targets = [("httpbin.org", 80), ("www.google.com", 80), ("www.github.com", 80)]
        ...
        for connection_id, host, port in connections:
            ...
            if stats:
                print(
                    f"Connection {connection_id} to {host}:{port} - Duration: {stats['duration']:.2f}s"
                )
